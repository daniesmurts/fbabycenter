...
{
	include('db.php');
	$statement = $connection->prepare("SELECT cover FROM all_items WHERE id = '$product_id'");
	$statement->execute();
...
{
		include('db.php');
	$statement = $connection->prepare("SELECT cover2 FROM all_items WHERE id = '$product_id'");
	$statement->execute();
	$result = $statement->fetchAll();
	foreach($result as $row)
	{
...
{
		include('db.php');
	$statement = $connection->prepare("SELECT cover3 FROM all_items WHERE id = '$product_id'");
	$statement->execute();
	$result = $statement->fetchAll();
	foreach($result as $row)
	{
...
{
	include('db.php');
	$statement = $connection->prepare("SELECT * FROM all_items");
	$statement->execute();
	$result = $statement->fetchAll();
